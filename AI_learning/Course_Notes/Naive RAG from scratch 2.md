...
- LLMs will tend to hallucinate when no info is available. This is easy to overlook and in some cases hard to detect
- Can't cite sources. So, info will be unverifiable
- Long-context prompting approach will not scale properly
- In an organization, there is no guarantee that all users will use the correct version of a document when they use long-context prompting
- RBAC cannot be ensured
### With RAG
- LLM reads chunks of proprietary docs at query time
- Knowledge as fresh as the corpus and not training dataset that the LLM was built on
